...
{	
	...
	public <T extends MysqlTable<T>> ArrayList<Integer> getMostCountWhereColumnAreSame(T t, String groupBy)
	{
		try (Connection conn = mysqlBaseSetup.getConnection();)
		{
			PreparedStatement ps = getPreparedStatement(conn,
					" SELECT `"+groupBy+"`, count(*) AS `Cnt` FROM `" + t.getMysqlTableName() + "` GROUP BY "+groupBy,
					1);
	        ResultSet rs = ps.executeQuery();
	        MysqlBaseHandler.addRows(QueryType.READ, rs.getMetaData().getColumnCount());
