package me.avankziar.sj.general.assistance;
...
{	
	...
	public static TextComponent generateTextComponent(String message)
	{
		...
		String sepw = "@";
		String sepspace = "+";
		TextComponent tc = ChatApiItem.tc("");
		...
		for(String word : space)
		{
			...
			if(word.contains(sepb))
			{
				...
				newtc = ChatApiItem.tctl(function[0].replace(sepspace, " ")+" ");
				if(function.length == 2)
				{
					if(function[1].contains(idhover))
					{
						String[] at = function[1].split(sepw);
						ChatApiItem.hoverEvent(newtc,HoverEvent.Action.valueOf(at[1]), at[2].replace(sepspace, " "));
					}
					if(function[1].contains(idclick))
					{
						String[] at = function[1].split(sepw);
						ChatApiItem.clickEvent(newtc,ClickEvent.Action.valueOf(at[1]), at[2].replace(sepspace, " "));
					}
				}
